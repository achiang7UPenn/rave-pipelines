...
```{rave give_conditions_analyzed, use_rave = TRUE, deps = "condition_groupings", export = "conditions_analyzed"}
condition_keys <- names(condition_groupings)
conditions_analyzed <- lapply(condition_keys, function(key)
  condition_groupings[[key]][['conditions']])
```
...
```{python}
conditions_analyzed = r.conditions_analyzed
...
```{python plot_fooof_fit, use_rave = FALSE, deps = c("power_outputs_list", "max_n_peaks", "aperiodic_mode", "freq_range")}
# title = None
freq_range = freq_range
max_n_peaks=max_n_peaks
plt_log = True
...
shared.plot_fooof_fits(
  ...
  aperiodic_mode,
  plt_log=plt_log,
  condition_names=conditions_analyzed
)
```
